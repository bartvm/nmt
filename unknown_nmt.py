...
def init_params(options):
    ...
    #MLP for context embedding! Takes input context
...
def build_model(tparams, options):
    ...
    #get the context for each of the unknown words
    unk_ctx = get_ctx_matrix(x_data, 5) 

    #get the mask for the original matrix
    mask_index_emb = get_mask_matrix(x, 1)
    ...
    unk_ctx_emb = get_layer('ff')[1](tparams, unk_ctx, options, 
				     prefix='ff_embedding', activ='tanh')


    unk_emb = tensor.matrix('unk_emb', dtype='float32')
    ...
    return trng, use_noise, x, x_mask, y, y_mask, opt_ret, cost, unk_emb
...
def build_sampler(tparams, options, trng):
    ...
    unk_ctx = tensor.matrix('unk_ctx', dtype='float32')
    mask_index_emb = tensor.matrix('mask_index_emb', dtype='float32')
