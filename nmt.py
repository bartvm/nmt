...
def train(dim_word=100,
    ...
    for eidx in xrange(max_epochs):
 	...
 	for x,y in train:
		...
		if numpy.mod(uidx, saveFreq) == 0:
                	...
                	if not overwrite:
                    		print 'Saving the model at iteration %d...' % uidx
                    		saveto_uidx = '%s.iter%s.npz' % (os.path.splitext(saveto)[0], uidx)
                    		numpy.savez(saveto_uidx, history_errs=history_errs, uidx=uidx, **unzip(tparams))
                    		print 'Done\n'
		...
		if numpy.mod(uidx, validFreq) == 0:
            	...
            	if uidx >= finish_after:
                	print 'Finishing after %d iterations!\n' % uidx
                	estop = True
                	break

        print 'Seen %d samples\n' % n_samples
	if estop:
            break
    ...
    numpy.savez(saveto, zipped_params=best_p, uidx=uidx, **params)
    return valid_err
