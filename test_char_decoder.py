...
if __name__ == '__main__':
    ...
    params = OrderedDict()
    params['Cemb_src'] = norm_weight(options['n_chars_src'],
                                     options['src_char_dim'])
    params['Cemb_trg'] = norm_weight(options['n_chars_trg'],
                                     options['trg_char_dim'])
    ...
    word_hidden_states = gru_layer(tparams,
                                   char_hidden_states[-1],
                                   options,
                                   prefix='word_enc')
    word_hidden_states = word_hidden_states[0]

    # compile theano functions
    f_encode_word = theano.function(inputs=[x], outputs=[char_hidden_states])
    f_encode_sent = theano.function(inputs=[x], outputs=[word_hidden_states])
    """
    ...
    src_sentences = np.concatenate(
        [
            src_sentence_1[:, :, None], src_sentence_2[:, :, None]
        ],
        axis=2)
    ...
    trg_sentences = np.concatenate(
        [
            trg_sentence_1[:, :, None], trg_sentence_2[:, :, None]
        ],
        axis=2)
